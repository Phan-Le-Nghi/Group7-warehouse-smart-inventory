...
| OQ-013 | Trigger, điều kiện trước, kết quả thành công, ngoại lệ và trạng thái hoàn tất của từng khu vực quy trình bắt buộc là gì? | `PARTIALLY DECIDED / OPEN`: `DEC-036` approve `RECEIVE_RECORDED`, Warehouse Staff mismatch acknowledgement và downstream Putaway eligibility guard, nhưng không canonicalize final Receive completion hoặc exact Receive -> Putaway handoff. `DEC-037` approve một immutable Pick result cho prepared/external request và typed conflict khi confirm lần hai; correction/reversal, future retry/idempotency và Manager partial-review contract vẫn ngoài current Pick slice. Putaway exception/downstream handoff, Transfer exception, Audit mismatch completion, Adjust rejected-case closure và các handoff chưa nêu rõ vẫn mở. |
| OQ-014 | Có hỗ trợ thực hiện một phần đối với Receive, Putaway, Pick hoặc Transfer không? | `OPEN` ở cấp cross-workflow: `DEC-037` đã quyết định current Pick slice cho phép một explicit `PARTIAL_INSUFFICIENT` result với `0 < picked_quantity < requested_quantity`, explicit user confirmation và không auto-allocation; multiple later partial confirmations ngoài slice. Partial Receive là `NOT IMPLEMENTED` trong current slice theo `DEC-036` nhưng không bị loại vĩnh viễn; partial Putaway/Transfer vẫn chưa quyết định. |
...
| OQ-013 | Có mô tả cấp cao từ research (`EVD-002` đến `EVD-017`), boundary Putaway/Transfer/Pick từ `DEC-009`, `RECEIVE_RECORDED`/reference-review eligibility từ `DEC-036` và current Pick confirmation lifecycle từ `DEC-037`. | Final Receive completion/exact Putaway handoff, future Pick retry/idempotency/correction/reversal/Manager review và các lifecycle gap khác vẫn chưa rõ; câu hỏi tiếp tục `PARTIALLY DECIDED / OPEN`. |
