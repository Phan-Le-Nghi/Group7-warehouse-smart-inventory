...
| OQ-013 | Trigger, điều kiện trước, kết quả thành công, ngoại lệ và trạng thái hoàn tất của từng khu vực quy trình bắt buộc là gì? | `PARTIALLY DECIDED / OPEN`: `DEC-036` approve `RECEIVE_RECORDED`, Warehouse Staff mismatch acknowledgement và downstream Putaway eligibility guard, nhưng không canonicalize final Receive completion hoặc exact Receive -> Putaway handoff. `DEC-037` approve một immutable Pick result cho prepared/external request và typed conflict khi confirm lần hai; correction/reversal, future retry/idempotency và Manager partial-review contract vẫn ngoài current Pick slice. `DEC-038` approve immutable Transfer confirmation, transactional safe replay và đặt correction/reversal ngoài `US-TRF-001`; broader Transfer failure/cancel lifecycle và future reversal contract vẫn mở. Putaway exception/downstream handoff, Audit mismatch completion, Adjust rejected-case closure và các handoff chưa nêu rõ vẫn mở. |
...
- `OQ-013` — `PARTIALLY DECIDED / OPEN`: Round 2 và `DEC-036–038` đã xác định nhiều trigger, precondition, action và outcome, gồm `RECEIVE_RECORDED`, Pick result và immutable Transfer confirmation/safe replay. Final Receive completion/handoff, broader Transfer failure/cancel/future reversal contract và các lifecycle gap còn lại được ghi tại bảng OQ và `workflow-overview.md`; không được suy diễn.
...
| OQ-013 | Có mô tả cấp cao từ research (`EVD-002` đến `EVD-017`), boundary Putaway/Transfer/Pick từ `DEC-009`, `RECEIVE_RECORDED`/reference-review eligibility từ `DEC-036`, current Pick confirmation lifecycle từ `DEC-037` và immutable Transfer confirmation/safe retry boundary từ `DEC-038`. | Final Receive completion/exact Putaway handoff, future Pick retry/idempotency/correction/reversal/Manager review, broader Transfer failure/cancel/future reversal contract và các lifecycle gap khác vẫn chưa rõ; câu hỏi tiếp tục `PARTIALLY DECIDED / OPEN`. |
