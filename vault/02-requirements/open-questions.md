...
| OQ-012 | Lot/batch, serial number, expiry date, unit of measure hoặc unit conversion có thuộc phạm vi không? | `OPEN`: `DEC-037`, `DEC-038`, `DEC-040` và `DEC-041` chỉ approve strict integer như implementation constraint cho current Pick, Transfer, original Audit và Audit-recheck slices. Các quyết định slice này không canonicalize global quantity representation; lot/batch, serial, expiry, UOM, decimal quantity và conversion vẫn mở. |
| OQ-013 | Trigger, điều kiện trước, kết quả thành công, ngoại lệ và trạng thái hoàn tất của từng khu vực quy trình bắt buộc là gì? | `PARTIALLY DECIDED / OPEN`: `DEC-036` approve `RECEIVE_RECORDED`, Warehouse Staff mismatch acknowledgement và downstream Putaway eligibility guard, nhưng không canonicalize final Receive completion hoặc exact Receive -> Putaway handoff. `DEC-037` approve một immutable Pick result cho prepared/external request và typed conflict khi confirm lần hai; correction/reversal, future retry/idempotency và Manager partial-review contract vẫn ngoài current Pick slice. `DEC-038` approve immutable Transfer confirmation, transactional safe replay và đặt correction/reversal ngoài `US-TRF-001`. `DEC-039` chỉ approve read-only confirmed Transfer history. `DEC-040` approve original Audit recording semantics. `DEC-041` approve Manager-only per-line mandatory recheck, current-stock snapshot, exactly-one durable `audit_rechecks` evidence, safe replay và Adjust-eligibility context without creating Adjust; original Audit giữ `MISMATCH_RECORDED`. Audit close/resolve/correction/reversal, broader Transfer lifecycle, Putaway exception/downstream handoff, Adjust rejected-case closure và các handoff chưa nêu rõ vẫn mở. |
...
| OQ-022 | Barcode/QR, scanner, sử dụng mobile/offline hoặc tích hợp bên ngoài có thuộc phạm vi không? | `OPEN`: current approved slices, gồm `US-AUD-001` tại `DEC-040` và `US-AUD-002` tại `DEC-041`, không thêm scanner/device, mobile/offline hoặc external integration behavior; việc loại khỏi current slices không giải quyết câu hỏi toàn cục. |
...
- `OQ-013` — `PARTIALLY DECIDED / OPEN`: Round 2 và `DEC-036–041` đã xác định nhiều trigger, precondition, action và outcome, gồm `RECEIVE_RECORDED`, Pick result, immutable Transfer confirmation/safe replay, read-only confirmed Transfer history, Audit `MATCH_COMPLETED`/`MISMATCH_RECORDED` recording semantics và exactly-one Manager recheck per mismatch line. Final Receive completion/handoff, broader Transfer lifecycle, Audit close/resolve/correction/reversal và các lifecycle gap còn lại được ghi tại bảng OQ và `workflow-overview.md`; không được suy diễn.
...
- `OQ-014` vẫn `OPEN`; partial Receive và partial Transfer là `NOT IMPLEMENTED` trong current slices, không phải permanently out. `DEC-040/041` loại device/integration khỏi current Audit slices nhưng không đóng `OQ-022`; các OQ AI chưa có quyết định mới vẫn `OPEN QUESTION`.
...
| OQ-013 | Có mô tả cấp cao từ research (`EVD-002` đến `EVD-017`), boundary Putaway/Transfer/Pick từ `DEC-009`, `RECEIVE_RECORDED`/reference-review eligibility từ `DEC-036`, current Pick confirmation lifecycle từ `DEC-037`, immutable Transfer confirmation/safe retry boundary từ `DEC-038`, read-only confirmed history contract từ `DEC-039`, Audit recording contract từ `DEC-040` và Manager recheck contract từ `DEC-041`. | Final Receive completion/exact Putaway handoff, future Pick retry/idempotency/correction/reversal/Manager review, broader Transfer failure/cancel/future reversal, Audit close/resolve/correction/reversal và các lifecycle gap khác vẫn chưa rõ; câu hỏi tiếp tục `PARTIALLY DECIDED / OPEN`. |
