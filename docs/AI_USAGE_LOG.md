...
### AI-USE-024 — US-AUD-001 Technical Story Spec human-review finalization

- **Task:** Audit the current application and canonical sources, prepare `US-AUD-001` for human review, then synchronize the approved implementation-ready Audit contract without application code, migration, test implementation, commit or push.
- **AI support:** AI mapped all four canonical Audit ACs; audited per-location stock, auth roles, neighboring workflows, frontend routing/API patterns, fixtures and CI; separated canonical facts from unresolved scope, snapshot, discrepancy, missing-balance, authorization, persistence and concurrency choices; and prepared the H1–H10 Human Decision Pack and exact implementation/test plan.
- **Human decisions:** Human approved one `AuditSession` with many unique `(sku_id, location_id)` `AuditLine` rows; `SELECTED_PAIRS` and current SKU × tracked-location `WHOLE_WAREHOUSE` scope; server-side Warehouse resolution; whole-scope membership conflict; submit-time set-based stock snapshot; missing valid balance as zero without creating `StockBalance`; strict integer current-slice quantity; persisted `physical - system` discrepancy; `MATCH_COMPLETED`/`MISMATCH_RECORDED`; Warehouse Staff-only context/submit; required Audit-specific atomic `Idempotency-Key` semantics; `/audits/new` one-page UI; and strict no-stock-effect behavior. These decisions are recorded at `DEC-040`.
- **Scope guard:** No Manager review/recheck, discrepancy resolution, Adjust CTA/action, Audit history/filter, stock correction, generic workflow/idempotency framework, scanner/device, notification, AI, application code, migration or automated test was added.
- **Open boundaries:** `OQ-012` remains open globally because strict integer is slice-specific. `OQ-013` remains `PARTIALLY DECIDED / OPEN` for broader Audit review/recheck/correction and lifecycle behavior; `US-AUD-002` retains that responsibility. `OQ-022` remains open for device, scanner, mobile/offline and integration behavior.
- **Verification boundary:** Documentation-only finalization. No build, lint, application test, migration cycle or browser test was run or claimed. Verification is limited to documentation diff checks requested for this task.
...
- **Human decisions:** Human explicitly approved `SKU_NOT_FOUND`, `LOCATION_NOT_FOUND`, `LOCATION_OUTSIDE_WAREHOUSE`, the PostgreSQL `INTEGER` upper bound, replay lookup before current scope/stock reads, one-statement whole-Warehouse membership/snapshot and the frontend key/reload lifecycle.
...
- **Task:** Audit the current `US-AUD-001` implementation and canonical Audit/Adjust boundaries, prepare `US-AUD-002` for human review, then synchronize the approved implementation-ready Manager recheck contract without application code, migration, test implementation, commit or push.
- **AI support:** AI mapped the three canonical ACs; audited Audit session/line persistence, current snapshot/result/status behavior, authorization, Manager read patterns, Adjust boundaries, frontend, fixtures, Playwright and CI; compared line-field, recheck-table and review-header persistence options; analyzed original-versus-current snapshot semantics, exactly-one concurrency, idempotency and no-stock-effect risks; and prepared H1–H12 plus the exact implementation/test/file plan.
- **Human decisions:** Human approved Manager as the direct recheck actor; `audit_line_id` discrepancy identity; current committed stock snapshot at submit with missing balance zero; separate `audit_rechecks`; exactly one recheck per mismatch line; `MATCH`/`MISMATCH` recheck result without changing `MISMATCH_RECORDED`; strict integer quantity; required Audit-recheck-specific atomic `Idempotency-Key` semantics and historical replay; line-level Adjust eligibility context without Adjust creation/application; `/audit-discrepancies` list/detail/recheck API and Manager UI; and strict original-Audit/stock/neighboring-workflow no-effect guarantees. These decisions are recorded at `DEC-041`.
- **Scope guard:** No Warehouse Staff recheck, repeated or session-batch recheck, original Audit mutation, Audit close/resolve/correction/reversal, StockBalance mutation, Adjust FK/request/application/CTA/navigation, generic review/workflow/idempotency framework, scanner/device, notification, AI, application code, migration or automated test was added.
- **Open boundaries:** `OQ-012` remains open globally because strict integer is slice-specific. `OQ-013` remains `PARTIALLY DECIDED / OPEN` for Audit close/resolve/correction/reversal and broader lifecycle behavior. `OQ-022` remains open for device, scanner, mobile/offline and integration behavior.
- **Verification boundary:** Documentation-only finalization. No build, lint, application test, migration cycle or browser test was run or claimed. Verification is limited to the requested documentation diff checks.
...
- **Human decisions:** Human approved no speculative `audit_lines` index; shared list/detail item shape; one-route master/detail UI without React Router; read-only detail refetch after `RECHECK_ALREADY_RECORDED`; exact migration constraints; conflict-target-free `ON CONFLICT DO NOTHING RETURNING`; and factual-only documentation reconciliation.
