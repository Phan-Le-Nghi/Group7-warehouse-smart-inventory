...
- Authentication/authorization foundation: PostgreSQL-backed session and database-role resolution approved at `DEC-031/033`; implementation is merged and CI verified, while staging HTTPS cookie behavior and PostgreSQL 17/Supabase verification remain pending.
- Deployment: Vercel frontend → Render FastAPI → Supabase PostgreSQL 17 is approved only for staging/demo at `DEC-032/034/035`; long-term production target remains TBD and no deployment is implemented.
