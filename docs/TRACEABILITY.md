...
| `US-AUD-002` | Manager-only discrepancy list/detail; exactly-one per-line current-stock recheck; immutable original Audit; no auto Adjust or stock effect | Implementation-ready Technical Story Spec approved at `DEC-041`; application code, migration and tests not started |
