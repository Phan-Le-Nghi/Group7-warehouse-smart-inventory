# Traceability v9 — Requirements + NFR + Audit Contract Baseline
...
|---|---|---|---|
| `REQ-001/002/004`, `CAND-REQ-003/010/011`, `FR-012` → `US-TRF-001` → `DEC-038` | Alembic `20260925_0005`; immutable `transfers`; atomic key claim; transactional destination materialization; shared Pick/Transfer UUID lock order; Warehouse Staff context/confirmation API; `/transfer/{sku_id}` UI; component, migration, concurrency and browser scenarios | 2026-09-25: backend Ruff lint/format PASS; local full pytest `131 passed, 9 PostgreSQL-only skipped`; frontend ESLint/TypeScript/Vitest `38 passed`/production build PASS; Playwright discovery lists 10 tests including `TEST-TRF-E2E-001…003`. | PostgreSQL 17/18 Transfer/Pick concurrency and migration evidence requires CI; browser execution is not locally verified; UI remains minimum technical UI pending human visual review. No history/query, correction/reversal, partial, bulk/multi-SKU, FIFO/FEFO, reservation, scanner/device, deployment or generic Movement. `OQ-012`, remaining `OQ-013`, `OQ-014`, `OQ-022` remain open. |
...
|---|---|---|---|
| `REQ-002/003/004`, `CAND-REQ-010`, `FR-013` → `US-TRF-002` → `DEC-039` | Dedicated history schemas/query; Manager-only `GET /api/v1/transfers`; server-enforced exactly-one-Warehouse invariant; exact nested Transfer/SKU/location/actor response; `transferred_at DESC, id DESC`; `/transfers/history` table; strict read-only/no-effect fixtures; existing indexes and no migration | 2026-09-26: backend Ruff lint/format PASS; full local pytest `142 passed, 9 PostgreSQL-only skipped`; frontend ESLint/TypeScript/Vitest `46 passed`/production build PASS; Playwright discovery lists 14 tests including `TEST-TRF2-E2E-001…004`. | PostgreSQL-backed browser execution is locally `NOT VERIFIED`; human diff/visual review pending. No idempotency/stock fields, mutation actions, multi-Warehouse membership, pagination, filters, search, export, detail page, correction/reversal or device/integration behavior. `OQ-013` and `OQ-022` remain open. |

## US-AUD-001 implementation contract

| Requirement / Story / decision trace | Approved implementation contract | Current evidence | Scope boundary / pending evidence |
|---|---|---|---|
| `REQ-002/004`, `CAND-REQ-003/005/010`, `CAND-BR-003/009` → `US-AUD-001` → `DEC-040` | One `AuditSession` with unique pair lines; `SELECTED_PAIRS` and server-resolved current SKU × tracked-location `WHOLE_WAREHOUSE`; submit-time set-based snapshot; missing balance zero without creation; strict integer; persisted `physical - system`; `MATCH_COMPLETED`/`MISMATCH_RECORDED`; Staff-only context/idempotent submit; `/audits/new`; strict no-stock-effect guarantee | Worktree implementation candidate: Alembic `20260926_0006`, Audit domain/API, one-page frontend, component/migration/concurrency/browser tests. Fresh local: Ruff PASS; pytest `164 passed, 13 PostgreSQL-only skipped`; frontend ESLint/TypeScript/Vitest `56 passed`/build PASS; Playwright discovery 20 tests including six Audit scenarios. PostgreSQL and real-browser execution remain pending. | Manager review/recheck/resolution remains `US-AUD-002`; no stock mutation, Adjust, history/filter, scanner/device, generic workflow/idempotency framework or AI. `OQ-012`, broader `OQ-013` and `OQ-022` remain open. |

## US-AUD-002 implementation contract

| Requirement / Story / decision trace | Approved implementation contract | Current evidence | Scope boundary / pending evidence |
|---|---|---|---|
| `REQ-002/004`, `CAND-REQ-005/010`, `CAND-BR-002/010` → `US-AUD-002` → `DEC-041` | Manager-only mismatch list/detail and exactly-one recheck per `audit_line_id`; current committed stock snapshot with missing balance zero; strict integer; separate immutable `audit_rechecks`; `MATCH`/`MISMATCH`; required atomic `Idempotency-Key` replay; original Audit remains `MISMATCH_RECORDED`; mismatch recheck exposes Adjust eligibility context only; `/audit-discrepancies`; strict no-effect guarantee | Worktree implementation candidate: Alembic `20260926_0007`, shared list/detail query, atomic recheck service/API, Manager master/detail UI and component/migration/concurrency/browser tests. Fresh local: Ruff PASS; pytest `191 passed, 18 PostgreSQL-only skipped`; frontend ESLint/TypeScript/Vitest `66 passed`/build PASS; Playwright discovery 25 tests including five US-AUD-002 scenarios. PostgreSQL 17/18 concurrency/migration, real browser execution and human diff/visual review remain pending. | No repeated or session-batch recheck, original Audit mutation, stock mutation, Adjust creation/application or automatic navigation, close/resolve/correction/reversal, generic workflow/idempotency framework, scanner/device or AI. `OQ-012`, remaining `OQ-013` and `OQ-022` remain open. |
...
|---|---|---|
| `US-REC-001` | actual entry/compare; match; quantity discrepancy; Warehouse Staff reference-mismatch acknowledgement and Putaway eligibility guard | Implemented in worktree under `DEC-036`; local backend `86 passed`, Ruff checks PASS, SQLite migration cycle PASS, frontend lint/typecheck/`23` Vitest/build PASS; Playwright discovery lists both Receive flows but browser execution, PostgreSQL 18 CI and PostgreSQL 17 staging evidence remain pending; `OQ-013/014` remain open |
...
| `US-PICK-001` | full Pick; multi-location; explicit `PARTIAL_INSUFFICIENT`; negative-stock guard | Implemented in worktree under `DEC-037`: two-table migration, locked atomic backend, Warehouse Staff API, two-step partial UI, component/integration/concurrency test coverage and isolated Playwright scenarios. Fresh local evidence: Ruff PASS; pytest `113 passed, 3 PostgreSQL-only skipped`; frontend ESLint/TypeScript/Vitest `30 passed`/build PASS; Playwright discovery lists 7 tests. PostgreSQL 17/18 concurrency and browser E2E execution remain pending. |
| `US-TRF-001` | source/destination effects; Warehouse total; minimum record; negative-stock guard | Implemented in worktree under `DEC-038`: immutable Transfer migration/model, atomic idempotency claim, missing-destination creation, shared Pick/Transfer lock ordering, Warehouse Staff API, minimum technical UI and component/migration/concurrency/E2E coverage. Local PostgreSQL concurrency and browser execution remain pending; human diff and visual review are required. |
| `US-TRF-002` | Manager history access; exact history fields; confirmation time; newest-first deterministic order; strict read-only behavior | Implemented in worktree under `DEC-039`: dedicated read query/schema, Manager authorization, exact-one-Warehouse resolution, table states, backend/component/no-effect tests and four discovered Playwright scenarios. PostgreSQL/browser execution and human review remain pending. |
| `US-AUD-001` | explicit selected pairs or whole-Warehouse full matrix; submit-time count/compare; durable result; `MATCH_COMPLETED` or `MISMATCH_RECORDED`; no stock effect | Implementation-ready Technical Story Spec approved at `DEC-040`; application code, migration and tests not started |
| `US-AUD-002` | Manager-only discrepancy list/detail; exactly-one per-line current-stock recheck; immutable original Audit; no auto Adjust or stock effect | Implemented in worktree under `DEC-041`: separate constrained `audit_rechecks`, shared list/detail read model, atomic safe replay/exactly-one service, Manager API/UI, component/migration/concurrency/E2E coverage and strict no-effect proofs. Fresh local checks pass; PostgreSQL 17/18 concurrency/migration, real Chromium execution and human review remain pending. |
...
|---|---|---|---|---|---|
| `REQ-001/002/003`, `CAND-REQ-001/002/009/010` | `US-REC-001` | [#7](https://tree.taiga.io/project/lenghi-group-07-project/us/7) / ID `9523822` — Ready | `T-REC-01` [#16](https://tree.taiga.io/project/lenghi-group-07-project/task/16); `T-REC-02` [#17](https://tree.taiga.io/project/lenghi-group-07-project/task/17); `T-REC-03` [#18](https://tree.taiga.io/project/lenghi-group-07-project/task/18) — New | `PF-01 — Receive → Putaway`; 10 wireframe states và 10 prototype counterparts human verified | Worktree implementation and local verification complete: backend 86 tests, frontend 23 tests/checks/build, SQLite migration cycle; awaiting human diff review, PostgreSQL 18/browser CI and later PostgreSQL 17 staging evidence |
