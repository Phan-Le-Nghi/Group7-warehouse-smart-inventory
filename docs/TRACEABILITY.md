...
| `US-TRF-001` | source/destination effects; Warehouse total; minimum record; negative-stock guard | Implementation-ready contract approved at `DEC-038`: immutable record, same-Warehouse different locations, destination creation, sorted UUID locking, idempotent retry, Warehouse Staff API and typed errors; no code/migration/test evidence yet |
