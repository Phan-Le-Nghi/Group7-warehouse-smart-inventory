# Traceability v8 — Requirements + NFR + Transfer Contract Baseline
...
|---|---|---|---|
| `REQ-002/003/004`, `CAND-REQ-010`, `FR-013` → `US-TRF-002` → `DEC-039` | Manager-only `GET /api/v1/transfers`; server-resolved canonical single Warehouse; exact nested Transfer/SKU/location/actor response; `transferred_at DESC, id DESC`; no pagination/filter; `/transfers/history` table; strict read-only/no-effect guarantees; reuse current indexes with no migration | Documentation-only human approval. Existing `US-TRF-001` foundation has durable Transfer records and relevant indexes; no history API/UI/test implementation evidence exists yet. | Implementation, fresh backend/frontend/Playwright verification and human diff review pending. No idempotency/stock fields, mutation actions, multi-Warehouse membership, pagination, filters, search, export, detail page, correction/reversal or device/integration behavior. `OQ-013` and `OQ-022` remain open. |
...
| `US-TRF-002` | Manager history access; exact history fields; confirmation time; newest-first deterministic order; strict read-only behavior | Implementation-ready contract approved at `DEC-039`; implementation and test evidence not started |
...
| `OQ-013` | PARTIALLY DECIDED / OPEN | `DEC-036` defines `RECEIVE_RECORDED` and reference-review eligibility; `DEC-037` defines the current one-result Pick confirmation lifecycle; `DEC-038` defines immutable Transfer success, transactional safe replay and keeps correction/reversal outside `US-TRF-001`; `DEC-039` adds only a read-only confirmed-history contract and does not decide lifecycle behavior. Final Receive completion/handoff, future Pick retry/idempotency/correction/reversal/Manager review, Putaway exception/handoff, broader Transfer failure/cancel/future reversal contract, Audit mismatch completion and Adjust rejected-case closure remain open |
...
| `OQ-014`, `OQ-021`, `OQ-022` | OPEN QUESTION | `DEC-037` resolves partial semantics only for the current Pick slice; partial Receive and partial Transfer are not implemented in their current slices and are not permanently excluded; partial Putaway remains undecided. Alert and device/integration behavior remain open |
