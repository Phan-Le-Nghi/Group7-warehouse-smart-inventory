...
| `US-AUD-001` | `REQ-002/004`, `CAND-REQ-003/005/010` | `CAND-BR-003/009` | `DEC-010/014/017/018/021/025/031/040` | Verified evidence `EVD-015/016` + HUMAN PRODUCT/TECHNICAL DECISIONS | `OQ-012` global quantity model, broader `OQ-013` review/recheck/correction lifecycle and `OQ-022` device/integration remain open; `US-AUD-002` owns Manager review/recheck |
...
|---|---|---|
| `OQ-012` | OPEN QUESTION | `DEC-037/038/040` use strict integer only as current Pick/Transfer/Audit slice constraints; UOM, decimal quantity, conversion behavior and precision/scale remain globally undecided |
| `OQ-011` | RESOLVED — HUMAN PRODUCT DECISION | `DEC-010–015`; per-location `system stock quantity` |
| `OQ-013` | PARTIALLY DECIDED / OPEN | `DEC-036` defines `RECEIVE_RECORDED` and reference-review eligibility; `DEC-037` defines the current one-result Pick confirmation lifecycle; `DEC-038` defines immutable Transfer success and safe replay; `DEC-039` adds only read-only Transfer history. `DEC-040` defines Audit submit-time snapshot, `MATCH_COMPLETED`/`MISMATCH_RECORDED`, safe replay and no-stock-effect semantics, while Manager recheck/review, discrepancy resolution, correction/reversal and broader Audit lifecycle remain open under `US-AUD-002` or future contract. Final Receive handoff, future Pick lifecycle, Putaway exception/handoff, broader Transfer lifecycle and Adjust rejected-case closure also remain open |
