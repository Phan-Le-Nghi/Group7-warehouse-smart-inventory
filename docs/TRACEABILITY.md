...
| `OQ-013` | PARTIALLY DECIDED / OPEN | `DEC-036` defines `RECEIVE_RECORDED` and reference-review eligibility; `DEC-037` defines the current one-result Pick confirmation lifecycle. Final Receive completion/handoff, future Pick retry/idempotency/correction/reversal/Manager review, Putaway exception/handoff, Transfer exception/reversal, Audit mismatch completion and Adjust rejected-case closure remain open |
...
| `OQ-014`, `OQ-021`, `OQ-022` | OPEN QUESTION | `DEC-037` resolves partial semantics only for the current Pick slice; partial Receive remains not implemented/not permanently excluded and partial Putaway/Transfer remain undecided. Alert and device/integration behavior remain open |
