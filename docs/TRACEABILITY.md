...
| `OQ-013` | PARTIALLY DECIDED / OPEN | `DEC-036` defines `RECEIVE_RECORDED` and reference-review eligibility; `DEC-037` defines the current one-result Pick confirmation lifecycle; `DEC-038` defines immutable Transfer success, transactional safe replay and keeps correction/reversal outside `US-TRF-001`. Final Receive completion/handoff, future Pick retry/idempotency/correction/reversal/Manager review, Putaway exception/handoff, broader Transfer failure/cancel/future reversal contract, Audit mismatch completion and Adjust rejected-case closure remain open |
